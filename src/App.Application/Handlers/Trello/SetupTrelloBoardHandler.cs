        ...
        public async Task<string> Handle(SetupTrelloBoardCommand boardCommand, CancellationToken cancellationToken)
        {
            var prompt = 
                $@"
                    ...
                    [ ""curl -X POST ""https://api.trello.com/1/boards/"" -H ""Content-Type: application/x-www-form-urlencoded"" -d ""name=My Board&key=YOUR_TRELLO_API_KEY&token=YOUR_TRELLO_TOKEN"""", ""curl -X POST ""https://api.trello.com/1/boards/YOUR_BOARD_ID/labels/"" -H ""Content-Type: application/x-www-form-urlencoded"" -d ""name=Project A&color=blue&key=YOUR_TRELLO_API_KEY&token=YOUR_TRELLO_TOKEN"""", ""curl -X POST ""https://api.trello.com/1/lists/"" -H ""Content-Type: application/x-www-form-urlencoded"" -d ""name=To Do&idBoard=YOUR_BOARD_ID&key=YOUR_TRELLO_API_KEY&token=YOUR_TRELLO_TOKEN"""", ""curl -X POST ""https://api.trello.com/1/lists/"" -H ""Content-Type: application/x-www-form-urlencoded"" -d ""name=In Progress&idBoard=YOUR_BOARD_ID&key=YOUR_TRELLO_API_KEY&token=YOUR_TRELLO_TOKEN"""", ""curl -X POST ""https://api.trello.com/1/lists/"" -H ""Content-Type: application/x-www-form-urlencoded"" -d ""name=Done&idBoard=YOUR_BOARD_ID&key=YOUR_TRELLO_API_KEY&token=YOUR_TRELLO_TOKEN"""" ]
